...
This research documents a simple but important finding: when big, attention-grabbing events happen — leaked documents, political scandals, viral media cycles — institutional policy changes quietly follow about two weeks later. This isn't speculation. It's a measured statistical relationship with a correlation of r = +0.6196, meaning the connection is strong, and a p-value of 0.0004, meaning there's less than a 0.05% chance the pattern is random noise.

Think of it like weather and umbrellas. You don't need a conspiracy to explain why umbrella sales spike after rain — people respond to the same environmental conditions independently. This project documents a similar phenomenon in politics and finance: friction events (document releases, investigations, scandals) and compliance events (policy shifts, deals, personnel changes) cluster around the same calendar windows — holidays, fiscal deadlines, congressional recesses — because multiple actors respond to the same low-attention periods without needing to coordinate.
